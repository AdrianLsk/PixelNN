...
from pixel_cnn import build_pixel_cnn
...
arg_parser.add_argument('--num-layers', type=int, default=3)
...
arg_parser.add_argument('--masked', action='store_true', default=False)
...
model, output_layer = build_pixel_cnn(
    input_shape=INPUT_SHAPE, n_layers=args.num_layers, masked=args.masked)
...
print('Loading data ...')
dataset = load_data(False, dataset) # data already shuffled
...
name = 'pixelcnn_{ds}_lr_{lr}_dlr_{dlr}_bs_{bs}_ep_{ep}'.format(
    ds=args.dataset.lower(), lr=LEARNING_RATE, dlr=LR_DECREASE, bs=BATCH_SIZE,
    ep=NUM_EPOCHS
)
